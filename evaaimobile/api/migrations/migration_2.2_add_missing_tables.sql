...
-- Using built-in gen_random_uuid() function for UUID generation (consistent with existing schema)
...
CREATE TABLE IF NOT EXISTS content (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    layer_id UUID REFERENCES layers(id) ON DELETE SET NULL, -- Контент может быть не привязан к слою
