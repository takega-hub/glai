    ...
    <div className="fixed inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white/10 border border-white/20 rounded-2xl p-6 w-full max-w-md relative shadow-xl shadow-purple-500/20">
            ...
            return (
              <div 
                ...
                className={`bg-white/5 rounded-xl p-4 border transition-all ${
                  canAfford
                    ? 'border-transparent hover:border-purple-400/50 hover:bg-white/10 cursor-pointer'
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={'bg-gradient-to-br from-purple-500/30 to-pink-500/30 p-3 rounded-lg'}>
                      <gift.icon className={'text-purple-200 w-6 h-6'} />
                    </div>
                    <div>
                      <h3 className="font-bold text-white text-lg">{gift.name}</h3>
                      <p className={'text-sm text-green-400'}>
                        +{gift.points} to trust
                      </p>
                    </div>
                  </div>
        ...
        <div className="text-center mt-6">
            <p className="text-purple-200">Your balance: <span className="font-bold text-white">{currentTokenBalance}</span> tokens</p>
        </div>
      </div>
    </div>
  );
